package creeperpookie.creeperoracledbplugin.util;
...
public class Utility
{
	...
	public static double getAttackDamage(ItemStack item, LivingEntity target, Collection<PotionEffect> activePotionEffects, boolean isCriticalHit)
	{
		...
		for (Enchantment enchantment : item.getEnchantments().keySet())
		{
			...
			{
				Bukkit.getScheduler().runTaskLater(CreeperOracleDBPlugin.getInstance(), () -> target.setFireTicks((80 * level) - 20), 20);
			}
	...
	public static void sendError(CommandSender commandSender, String message)
	{
		...
		if (commandSender instanceof Player player) player.playSound(player.getLocation(), Sound.ENTITY_ENDERMAN_TELEPORT, SoundCategory.PLAYERS, 1f, (float) (0.1 + ((CreeperOracleDBPlugin.getRandom().nextInt(2) == 0 ? -1 : 1) * (CreeperOracleDBPlugin.getRandom().nextDouble() / 20))));
	}
	...
	public static int getEnchantingLevel(int lightLevel)
	{
		...
		CreeperOracleDBPlugin.getInstance().getLogger().info("Calculating enchanting level for light level " + lightLevel);
	...
	public static void printException(Logger logger, Exception exception)
	{
		...
		for (StackTraceElement traceElement : exception.getStackTrace()) logger.severe("\tat " + traceElement);

		// Print suppressed exceptions, if any
		for (Throwable se : exception.getSuppressed()) logger.severe(se.getMessage());

		// Print cause, if any
		Throwable ourCause = exception.getCause();
		if (ourCause != null) logger.severe(exception.getMessage());
	}
}
